...
function pickKid(request) {
  const kidRaw = (request.headers.get("X-SR-Kid") || "K1").trim().toUpperCase();
  if (!/^K\d+$/.test(kidRaw)) return null;
  return kidRaw;
}

function pickSecret(env, kid) {
  const byKid = envStr(env, `SR_TRIGGER_SECRET_${kid}`, "");
  if (byKid) return { secret: byKid, source: `SR_TRIGGER_SECRET_${kid}` };

  const legacy = envStr(env, "SR_TRIGGER_SECRET", "");
  if (legacy) return { secret: legacy, source: "SR_TRIGGER_SECRET" };
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/health" || url.pathname === "/health/") {
      ...
      return json(200, { ok: true, service: `${repoName}-agent-trigger` });
    }
    ...
    if (!kid) return bad(401, "BadKid", "Invalid X-SR-Kid (expected K1, K2, ...)");
    ...
    if (!constantTimeEqual(expected, sigHeader)) {
      const debugEnabled = envStr(env, "SR_DEBUG", "0") === "1";
      if (debugEnabled) {
...
async function handleAuthorized(env, payload) {
  ...
  const devMode = envStr(env, "SR_DEV_MODE", "0") === "1";
  ...
  if (!token) {
    if (devMode) return json(200, { ok: true, verified: true, dispatched: false, reason: "SR_DEV_MODE=1 and GH_TOKEN not set" });
