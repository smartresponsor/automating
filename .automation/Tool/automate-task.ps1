...
switch ($Task) {
    ...
    "plan" {
        ...
        Set-Content -Path "report/agent-plan.json" -Value $json -Encoding UTF8

        Write-Host "OK plan: report/agent-plan.json"
        exit 0
    }

    "codex" {
        Ensure-Dir -Path ".github/prompts"
        if (-not (Test-Path ".github/prompts/repo-portrait-rwe.prompt.md")) {
            Copy-Item -Path "tool/template/prompt/repo-portrait-rwe.prompt.md" -Destination ".github/prompts/repo-portrait-rwe.prompt.md"
        }
        Write-Host "OK codex: .github/prompts/repo-portrait-rwe.prompt.md"
        exit 0
    }
