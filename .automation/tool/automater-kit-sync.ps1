...
param(
    ...
    [string]$AssetZip = $( if ($env:AUTOMATER_KIT_ASSET_ZIP)
{
    $env:AUTOMATER_KIT_ASSET_ZIP
}
else
{
    "automater-kit.zip"
} ),
    [string]$AssetSha = $( if ($env:AUTOMATER_KIT_ASSET_SHA)
{
    $env:AUTOMATER_KIT_ASSET_SHA
}
else
{
    "automater-kit.sha256"
} ),
    [string]$LockPath = $( if ($env:AUTOMATER_KIT_LOCK_PATH)
{
    $env:AUTOMATER_KIT_LOCK_PATH
}
else
{
    ".automation/automater-kit.lock.json"
} ),
    [string]$BranchPrefix = $( if ($env:AUTOMATER_KIT_BRANCH_PREFIX)
{
    $env:AUTOMATER_KIT_BRANCH_PREFIX
}
else
{
    "automater-kit"
} ),
    [string]$BaseBranch = $( if ($env:AUTOMATER_BASE_BRANCH)
{
    $env:AUTOMATER_BASE_BRANCH
}
else
{
    "master"
...
$readToken = $( if (-not [string]::IsNullOrWhiteSpace($env:AUTOMATER_SOURCE_TOKEN))
{
    $env:AUTOMATER_SOURCE_TOKEN
}
else
{
    $writeToken
...
{
    throw "No tag_name in latest release response."
}
...
{
    throw "Missing downloaded asset: $AssetZip"
}
if (-not(Test-Path -LiteralPath $shaPath))
{
    throw "Missing downloaded asset: $AssetSha"
}
...
{
    throw "Invalid kit zip: missing top-level folder 'automater-kit'."
}
