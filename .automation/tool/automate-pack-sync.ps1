...
function ParseIsoDurationSeconds([string]$Dur)
{
    ...
    if (-not$m.Success)
    {
        throw "Invalid ISO duration: $Dur (expected PnD or PTnHnM or PnDTnHnM)"
    }
...
function ShouldThrottle([string]$PackId, [string]$TimerIso)
{
    ...
    {
        if ([string]::IsNullOrWhiteSpace($triggerPack) -or $triggerPack -eq $PackId)
        {
            return $false
        }
    }
    ...
    try
    {
        $last = [DateTimeOffset]::Parse([string]$lock.appliedAt)
    }
    catch
    {
        return $false
...
function GetLatestTag([string]$Owner, [string]$Repo, [string]$ReadToken)
{
    ...
    {
        throw "No tag_name for $Owner/$Repo latest release."
    }
    return $tag
}
...
function DownloadAssets([string]$Owner, [string]$Repo, [string]$Tag, [string]$ZipName, [string]$ShaName, [string]$ReadToken)
{
    ...
    {
        Remove-Item -Recurse -Force -LiteralPath $dlDir
    }
    ...
    {
        throw "Missing asset $ZipName in $Owner/$Repo@$Tag"
    }
    if (-not(Test-Path -LiteralPath $shaPath))
    {
        throw "Missing asset $ShaName in $Owner/$Repo@$Tag"
    }
...
function ExtractZip([string]$ZipPath, [string]$ToDir)
{
    ...
    {
        Remove-Item -Recurse -Force -LiteralPath $ToDir
    }
...
{
    throw "Missing packs config: $PacksPath"
}
...
if (-not$config)
{
    throw "Invalid packs config: $PacksPath"
}
...
if ($packs.Count -eq 0)
{
    Write-Host "No packs configured."; exit 0
}

$baseBranch = $( if ($env:AUTOMATE_BASE_BRANCH)
{
    [string]$env:AUTOMATE_BASE_BRANCH
}
elseif ($defaults.baseBranch)
{
    [string]$defaults.baseBranch
}
else
{
    "master"
...
$readToken = $( if (-not [string]::IsNullOrWhiteSpace([string]$env:AUTOMATE_SOURCE_TOKEN))
{
    [string]$env:AUTOMATE_SOURCE_TOKEN
}
else
{
    $writeToken
...
$globalTimer = $( if ($env:AUTOMATE_PUSH_TIMER)
{
    [string]$env:AUTOMATE_PUSH_TIMER
}
elseif ($defaults.pushTimer)
{
    [string]$defaults.pushTimer
}
else
{
    "PT6H"
...
foreach ($pack in $packs)
{
    ...
    if (-not [string]::IsNullOrWhiteSpace($OnlyId) -and $id -ne $OnlyId)
    {
        continue
    }
    ...
    $zipName = $( if ($pack.source.assetZip)
    {
        [string]$pack.source.assetZip
    }
    elseif ($defaults.assetZip)
    {
        [string]$defaults.assetZip
    }
    else
    {
        "automate-kit.zip"
    } )
    $shaName = $( if ($pack.source.assetSha)
    {
        [string]$pack.source.assetSha
    }
    elseif ($defaults.assetSha)
    {
        [string]$defaults.assetSha
    }
    else
    {
        "automate-kit.sha256"
    } )
    $topFolder = $( if ($pack.source.topFolder)
    {
        [string]$pack.source.topFolder
    }
    elseif ($defaults.topFolder)
    {
        [string]$defaults.topFolder
    }
    else
    {
        ""
    } )
    $targetRoot = $( if ($pack.apply.targetRoot)
    {
        [string]$pack.apply.targetRoot
    }
    elseif ($defaults.targetRoot)
    {
        [string]$defaults.targetRoot
    }
    else
    {
        "."
    ...
    $timer = $( if ($pack.apply.pushTimer)
    {
        [string]$pack.apply.pushTimer
    }
    else
    {
        $globalTimer
    ...
    {
        throw "Invalid payload root for $id at $payloadRoot"
    }
...
{
    $parts += ("{0}@{1}" -f $a.id, $a.tag)
}
$msg = "automate pack sync: " + ($parts -join "; ")
if ($parts.Count -eq 0)
{
    $msg = "automate pack sync"
}
