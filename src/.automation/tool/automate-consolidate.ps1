...
# Consolidate legacy top-level folders into .automating-only layout.
...
Push-Location $Root
try {
  if (-not (Test-Path -LiteralPath ".automating")) {
    throw "Missing .automating. Apply the kit patch first."
  }
  ...
  $stash = Join-Path ".automating" "legacy"
  ...
  if ($RemoveLegacy) {
    Write-Host "Legacy folders moved under .automating/legacy."
  } else {
    Write-Host "Done. Review .automating/legacy and commit."
  }
} finally {
  Pop-Location
}
