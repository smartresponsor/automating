...
param(
  ...
  [string]$BaseBranch  = $(if ($env:AUTOMATE_BASE_BRANCH) { $env:AUTOMATE_BASE_BRANCH } else { "master" }),
  [string]$PushTimer   = $(if ($env:AUTOMATE_PUSH_TIMER) { $env:AUTOMATE_PUSH_TIMER } elseif ($env:AUTOMATER_PUSH_TIMER) { $env:AUTOMATER_PUSH_TIMER } else { "PT6H" }),
  [string]$ForceTag    = $env:AUTOMATE_FORCE_TAG,
  [string]$AssetZip    = "automate-kit.zip",
  [string]$AssetSha    = "automate-kit.sha256",
  [string]$LockPath    = ".automate/lock/kit.json"
)
...
function TryParseDuration([string]$s) {
  try { return [System.Xml.XmlConvert]::ToTimeSpan($s) } catch { return [TimeSpan]::FromHours(6) }
}
...
$readToken = $(if (-not [string]::IsNullOrWhiteSpace($env:AUTOMATE_SOURCE_TOKEN)) { $env:AUTOMATE_SOURCE_TOKEN } else { $writeToken })
...
if ([string]::IsNullOrWhiteSpace($tag)) {
  ...
  if ([string]::IsNullOrWhiteSpace($tag)) { throw "No tag_name in latest release." }
}
...
if (-not (Test-Path -LiteralPath $zipPath)) { throw "Missing asset: $AssetZip" }
if (-not (Test-Path -LiteralPath $shaPath)) { throw "Missing asset: $AssetSha" }
...
# Backup current .automate (if exists)
$backupRoot = Join-Path ".automate/backup" $tag
if (Test-Path -LiteralPath ".automate") {
  EnsureDir $backupRoot
  Copy-Item -Recurse -Force -LiteralPath ".automate" -Destination $backupRoot
}
...
$payload = Join-Path $extractDir ".automate"
if (-not (Test-Path -LiteralPath $payload)) { throw "Invalid kit zip: missing .automate folder at root." }

# Apply (overwrite)
Copy-Item -Recurse -Force -LiteralPath $payload -Destination ".automate"
